...
Used by the GitHub Action (and can be run locally to refresh the token) so the
pre-market report can pull live data from Fyers.
...
On success it writes FYERS_ACCESS_TOKEN back into .env, prints OK, and returns
the token string.

One-time prerequisite: the app must have been authorised once in a browser, and
External 2FA TOTP must be enabled at https://myaccount.fyers.in/ManageAccount.
"""
...
def generate_access_token(verbose: bool = True) -> str:
    env = load_env()
    ...
    # 4) exchange for an auth_code via the v3 token endpoint
